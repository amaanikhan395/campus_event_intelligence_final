...
function renderCards(summary) {
  const cards = [
    ['Total Rutgers Events', num(summary.total_events)],
...
function renderInsights(data) {
  const insights = [
    ['Top campus attendance driver', `${data.attendance_driver?.event_category || 'n/a'} · ${num(data.attendance_driver?.attended)} attended`],
...
function drawBarChart(canvasId, rows, labelKey, valueKey) {
  ...
  const chartHeight = height - padding * 1.65;
  ...
  rows.forEach((row, index) => {
    ...
    gradient.addColorStop(0, scarlet);
    gradient.addColorStop(1, scarletDark);
    ...
    ctx.fillText(label, x, height - 8 * ratio);
  });
}
...
init().catch((error) => {
  console.error(error);
  document.body.insertAdjacentHTML('afterbegin', '<div class="error-banner">Could not load data. Run <code>npm run reset</code> and <code>npm start</code> from the backend folder.</div>');
});
